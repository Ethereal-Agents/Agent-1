...
@pytest.fixture
def mock_config():
    with patch("agent.loop.get_system_prompt", return_value="sys prompt"), \
...
class TestAgentLLM:
    ...
    def test_call_llm_success(self, mock_config):
        ...
        with patch("agent.loop.litellm.completion", return_value=resp), \
    ...
    def test_call_llm_sliding_window_cache_string(self, mock_config):
        ...
        with patch("agent.loop.litellm.completion", return_value=resp) as mock_completion, \
...
class TestAgentRun:
    def test_finalize_run(self, mock_config):
        ...
        with patch("agent.loop.time.time", return_value=10.0), \
    ...
    def test_run_loop_max_steps(self, mock_config):
        ...
        with patch.object(agent, "_call_llm", return_value=msg_with_tool), \
    ...
    def test_run_loop_no_tools(self, mock_config):
        ...
        with patch.object(agent, "_call_llm", return_value=msg), \
             patch.object(agent, "_finalize_run"):
            agent.run("test issue")
    ...
    def test_run_loop_should_not_continue(self, mock_config):
        agent = Agent()
        msg_with_tool = MockMessage("reasoning", tool_calls=[MockToolCall("1", "submit_patch", "{}")])
