...
def main():
    ...
    parser.add_argument(
        "--easy", type=int, default=0, help="Number of easy samples (< 20 lines changed)."
    )
    parser.add_argument(
        "--medium", type=int, default=0, help="Number of medium samples (20-100 lines changed)."
    )
    parser.add_argument(
        "--hard", type=int, default=0, help="Number of hard samples (> 100 lines changed)."
    )
    parser.add_argument(
        "--output-dir",
        "-o",
    ...
    if args.easy == 0 and args.medium == 0 and args.hard == 0:
        print(
            "Error: You must specify at least one difficulty level to sample from (e.g., --easy 10)"
        )
        return
    ...
    def sample_pool(pool, requested, name):
        ...
        if requested > len(pool):
            print(
                f"Warning: Requested {requested} {name} samples, but only found {len(pool)}. Returning all available."
            )
            return pool.copy()
